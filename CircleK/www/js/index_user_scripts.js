 ...
 */
 function register_event_handlers()
 {
    ...
    $(document).on("click", "#pendingButton", function(evt)
    {
        intel.xdk.services.PendingList({"URL":"https://api.circlek.com/?pending&a="+un+":"+pw}).then(function(data) {
            var selectList = document.getElementById("selecttickets");
            alert(data.rss.api.ticket[1].ticketno);
            for(var i=0; i < data.rss.api.records; i++) {
                 alert("ticket No "+data.rss.api.ticket[i].ticketno);
                selectList.options[selectList.options.length]= new Option(data.rss.api.ticket[i].ticketno+": "+data.rss.api.ticket[i].title, data.rss.api.ticket[i].ticketno);
                alert("Loop Done");
             }
            });
